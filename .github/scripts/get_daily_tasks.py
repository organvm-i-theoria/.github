...
def get_daily_tasks(orchestration_file='.github/orchestration_tasks.json'):
    ...
    try:
        ...
        print(f"Error: Orchestration file not found: {orchestration_file}", file=sys.stderr)
        return []
    ...
    for task in config.get('tasks', []):
        ...
        if not croniter.is_valid(schedule):
            print(f"Warning: Invalid cron schedule for task {task.get('id', 'unknown')}: {schedule}", file=sys.stderr)
            continue
