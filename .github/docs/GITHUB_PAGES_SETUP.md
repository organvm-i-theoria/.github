...
The GitHub Pages gallery is an automatically generated website that showcases
all application walkthroughs and tutorials from the Ivviiviivvi organization. It
provides a searchable, filterable interface for browsing video tutorials and
...
Search is client-side JavaScript, providing instant results without page
reloads.
...
1. Verify walkthrough artifacts exist in source repositories
...
1. Check video file size (GitHub has 2GB artifact limit)
...
1. Check browser console for errors
1. Ensure `walkthrough_gallery.html` include is correct
...
1. Clear DNS cache: `ipconfig /flushdns` (Windows) or
...
1. Validate YAML syntax in `_config.yml`
1. Ensure all required dependencies are in `Gemfile`
1. Check for liquid syntax errors in templates
1. Run `bundle exec jekyll build` locally to debug
...
1. Use absolute paths: `/assets/images/logo.png`
1. Verify files are committed to repository
1. Check `.gitignore` is not excluding assets
1. Clear browser cache
...
1. Add front matter:
   ...
   ---
   ```
1. Add content in Markdown
1. Commit and push
1. Page will be available at `/new-page/`
...
1. Add filter in repository query:
   ```bash
...
**Solution**: Yes, edit `docs/_includes/walkthrough_gallery.html`:
