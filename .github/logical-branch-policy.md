...
Comprehensive policy and implementation guide for maintaining a
single-main-branch repository model with strict branch hygiene and lifecycle
management.
...
**Purpose**: This policy establishes a "main only" branch model where all
development work flows through short-lived feature branches that merge directly
into the main branch. This approach maximizes simplicity, reduces merge
conflicts, and ensures production-ready code is always deployable.

**Key Principles**:

1. **One permanent branch**: `main` (or `master`) is the only long-lived branch
1. **Short-lived feature branches**: Created from `main`, merged back to `main`,
   then deleted
1. **No develop branch**: Direct integration into `main` with proper CI/CD gates
1. **Continuous deployment**: Every merge to `main` represents a potential
   production release
...
- ⚠️ Projects requiring long-term support for multiple versions (use maintenance
  branches)
...
- No other long-lived branches are permitted (e.g., no `develop`, `staging`,
  `qa`)
- **Exception**: Temporary maintenance branches for critical patches (must be
...
- **Merge Commit (`--no-ff`)**: For complex features requiring preserved history
  (requires approval)
...
1. **Tag if Release**: If this merge represents a release, tag it:
   `git tag -a v1.2.3 -m "Release 1.2.3"`
1. **Delete Branch**: Remove the merged feature branch (automated via GitHub
   settings or manual)
1. **Notify Team**: CI/CD system notifies relevant channels (Slack, Teams,
   email)
...
- **"Resolved" is not enough**: Author must reply explaining the fix or
  rationale
...
- Merged branches are automatically deleted via GitHub settings
...
1. **Week 1 (Day 14)**: Automated notification to branch owner
   - Message: "Your branch `feature/X` has been inactive for 14 days. Please
     update or close."
1. **Week 2 (Day 21)**: Escalation to team lead
...
Identify all existing branches, classify them, and determine their fate (merge,
archive, delete).
...
**File**: `reports/branch-inventory-YYYY-MM-DD.csv` **Columns**: Branch, Last
Commit Date, Author, Status, Classification, Action, Deadline, Notes
...
**File**: `reports/branch-cleanup-summary-YYYY-MM-DD.md` **Sections**: Executive
Summary, Statistics, Actions Taken, Remaining Branches
...
**File**: `BRANCH_ARCHIVE_LOG.md` (root of repository) **Purpose**: Historical
record of all archived branches
...
1. Check ☑️ **Automatically delete head branches**
...
1. Configure settings:
   ...
   - ☑️ **Restrict who can push to matching branches** (optional, for
     high-security repos)
...
1. **Base**: `main`
1. **Compare**: `feature/your-branch`
1. Click **Create pull request**
1. **Title**: Follow conventional commit format (e.g.,
   `feat: add user authentication`)
1. **Description**: Fill out PR template (if available)
1. **Reviewers**: Assign reviewers
1. **Labels**: Add appropriate labels (`feature`, `bug`, `hotfix`)
1. Click **Create pull request**
...
1. Verify at least one approval ✅
...
1. Edit commit message if needed
1. Click **Confirm squash and merge**
1. (Optional) Click **Delete branch** if not auto-enabled

### Find Stale Branches (UI)

1. Navigate to repository
1. Click **Branches** (below repository name or via `/branches`)
1. Click **All branches**
1. Sort by **Last updated** (click column header)
1. Review branches not updated recently
1. Click **Delete** icon for stale branches (or **...** → **Delete branch**)
...
1. Push renamed branch: `git push origin archive/2024-12-old-branch`
...
**Weekly Notification** (see
[Branch Cleanup Workflow](#2-branch-cleanup-workflow))
...
**⚠️ Warning**: Auto-deletion is aggressive. Use with caution. Consider
...
**Responsibility**: Repository maintainers **Process**:

1. Run branch inventory script (see
   [Branch Inventory and Classification](#branch-inventory-and-classification))
1. Review all branches manually
1. Contact owners of non-compliant branches
1. Archive or delete branches after confirmation
1. Update `BRANCH_ARCHIVE_LOG.md`

#### PR Review Checklist

**Responsibility**: Code reviewers **Checklist** (add to PR template):
...
- ✅ Use pre-commit hooks to scan for secrets (e.g., `truffleHog`,
  `detect-secrets`)
