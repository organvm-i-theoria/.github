...
class TestIncidentResponseCLI:
        ...
        with patch(
            ...
            with patch("incident_response.load_config") as mock_config:
                mock_config.return_value = IncidentConfig(
                    create_github_issues=False, auto_execute_runbooks=False
                )
                with patch("incident_response.GitHubAPIClient"):
                    with patch.object(
                        IncidentResponseEngine, "_send_incident_notification"
                    ):
    ...
    def test_execute_runbook_cli(self, tmp_path, capsys):
        ...
        with patch(
            ...
            with patch("incident_response.load_config") as mock_config:
                ...
                with patch("incident_response.GitHubAPIClient"):
                    with patch.object(
                        IncidentResponseEngine, "_load_incident", return_value=incident
                    ):
    ...
    def test_update_status_requires_incident_id(self, capsys):
        ...
        with patch(
            "sys.argv", ["incident_response.py", "--update-status", "resolved"]
        ):
            with patch("incident_response.load_config") as mock_config:
                mock_config.return_value = IncidentConfig()
                with patch("incident_response.GitHubAPIClient"):
