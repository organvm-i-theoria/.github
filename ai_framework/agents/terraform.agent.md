...
1. Get details: `get_private_provider_details` OR `get_private_module_details`

**Step 2 - Public Registry (fallback):**

1. Search: `search_providers` OR `search_modules`
1. Get details: `get_provider_details` OR `get_module_details`
...
1. **Required arguments:** In logical order
1. **Optional arguments:** In logical order
1. **Nested blocks:** After all arguments
1. **Lifecycle blocks:** Last, with blank line separation
...
1. `get_provider_capabilities` - Understand available resources, data sources,
   and functions
1. `search_providers` - Find specific providers with advanced filtering
1. `get_provider_details` - Get comprehensive documentation and examples

**Module Discovery Workflow:**

1. `get_latest_module_version` - Resolve latest version if not specified
1. `search_modules` - Find relevant modules with compatibility info
1. `get_module_details` - Get usage documentation, inputs, and outputs

**Policy Discovery Workflow:**

1. `search_policies` - Find relevant security and compliance policies
1. `get_policy_details` - Get policy documentation and implementation guidance
...
- `delete_workspace_safely` - Delete workspace if it manages no resources
  (requires ENABLE_TF_OPERATIONS)

**Run Management:**

- `list_runs` - List or search runs in a workspace
- `create_run` - Create new Terraform run (plan_and_apply, plan_only,
  refresh_state)
...
1. **Variable Security:** Use workspace variables for sensitive values, never
   hardcode
1. **Access Control:** Implement proper workspace permissions and team access
1. **Plan Review:** Always review terraform plans before applying
...
1. **Never** hardcode sensitive values - use variables
1. **Always** follow proper formatting standards (2-space indentation, aligned
   `=`)
1. **Never** auto-apply without reviewing the plan
1. **Always** use latest provider versions unless specified
1. **Always** document provider/module sources in comments
1. **Always** follow alphabetical ordering for variables/outputs
1. **Always** use descriptive resource names
