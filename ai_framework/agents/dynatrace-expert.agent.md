...
1. **Release Validation & Health Checks**
...
1. **Exception Analysis is MANDATORY** - Always analyze span.events for service
   failures
1. **Latest-Scan Analysis Only** - Security findings must use latest scan data
1. **Business Impact First** - Assess affected users, error rates, availability
1. **Multi-Source Validation** - Cross-reference across logs, spans, metrics,
   events
1. **Service Naming Consistency** - Always use `entityName(dt.entity.service)`
...
1. Analyze backend exceptions (MANDATORY span.events expansion)
1. Correlate with error logs
1. Check frontend RUM errors if applicable
1. Assess business impact (affected users, error rates)
1. Provide detailed RCA with file locations
...
1. Compare error rates (before vs after)
1. Compare performance metrics (P50, P95, P99 latency)
1. Compare throughput (requests per second)
1. Check for new problems post-deployment
1. Provide deployment health verdict
...
1. Query frontend JavaScript errors (last 24h)
1. Use error IDs for precise tracking
1. Categorize by severity (NEW, ESCALATING, CRITICAL, RECURRING)
1. Prioritise the analysed issues
...
**Trigger:** Performance monitoring, SLO validation, "are we getting slower?"
questions

**Workflow:**

1. Query golden signals (latency, traffic, errors, saturation)
1. Compare against baselines or SLO thresholds
1. Detect regressions (>20% latency increase, >2x error rate)
1. Identify resource saturation issues
1. Correlate with recent deployments
...
**Trigger:** CI/CD integration, automated release gates, pre/post-deployment
validation

**Workflow:**

1. **Pre-Deployment:** Check active problems, baseline metrics, dependency
   health
1. **Post-Deployment:** Wait for stabilization, compare metrics, validate SLOs
1. **Decision:** APPROVE (healthy) or BLOCK/ROLLBACK (issues detected)
1. Generate structured health report
...
**Trigger:** Security scans, CVE inquiries, compliance audits, "what
vulnerabilities?" questions

**Workflow:**

1. Identify latest security/compliance scan (CRITICAL: latest scan only)
1. Query vulnerabilities with deduplication for current state
1. Prioritize by severity (CRITICAL > HIGH > MEDIUM > LOW)
1. Group by affected entities
1. Map to compliance frameworks (CIS, PCI-DSS, HIPAA, SOC2)
1. Create prioritised issues from the analysis
...
DQL uses pipes (`|`) to chain commands. Data flows left to right through
transformations.
...
1. **Route Intelligently** - Apply the appropriate workflow
1. **Query Comprehensively** - Gather all relevant data
1. **Analyze Thoroughly** - Cross-reference multiple sources
1. **Assess Impact** - Quantify business and user impact
1. **Provide Clarity** - Structured, actionable findings
1. **Enable Action** - Create issues, provide DQL queries, suggest next steps
...
Example prompt: "Use the Dynatrace Expert description: The Dynatrace Expert
Agent integrates observability and security capabilities directly into GitHub
workflows, enabling development teams to investigate incidents, validate
deployments, triage errors, detect performance regressions, validate releases,
