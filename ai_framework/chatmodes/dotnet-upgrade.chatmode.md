...
Use these instructions when implementing upgrade plans to ensure proper
...
1. Run a discovery pass to enumerate all `*.sln` and `*.csproj` files in the
   repository.
1. Detect the current .NET version(s) used across projects.
1. Identify the latest available stable .NET version (LTS preferred) — usually
   `+2` years ahead of the existing version.
1. Generate an upgrade plan to move from current → next stable version (e.g.,
   `net6.0 → net8.0`, or `net7.0 → net9.0`).
1. Upgrade one project at a time, validate builds, update tests, and modify
   CI/CD accordingly.
...
- `TargetFramework` starts with `netcoreapp`, `net5.0+`, `net6.0+`, etc. →
  **Modern .NET**
...
1. **Next:** Shared components and common utilities.
1. **Then:** API, Web, or Function projects.
1. **Finally:** Tests, integration points, and pipelines.
...
1. **Edit `<TargetFramework>`** in `.csproj` to the suggested version (e.g.,
   `net9.0`)
1. **Restore & update packages:**
   ```bash
   dotnet restore
...
1. **Build & test:**
   ...
   ```
1. **Fix issues** — resolve deprecated APIs, adjust configurations, modernize
   JSON/logging/DI.
...
1. "Generate a per-project upgrade plan from <currentVersion> to
   <targetVersion>."
1. "Suggest .csproj and pipeline edits to upgrade <ProjectName>."
