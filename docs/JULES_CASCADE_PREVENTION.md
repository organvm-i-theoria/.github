...
This system prevents cascading/redundant Jules tasks and consolidates all automated PRs into a single daily PR for streamlined review and approval.

## Problem Solved

**Before:**
- Multiple daily workflows running independently creating 10-100+ redundant tasks
...
2. Orchestrates daily tasks with deduplication
3. Processes task queue
4. Waits for tasks to complete (30 minutes)
5. Triggers PR consolidation
6. Cleans up old task records
...
2. Creates single consolidation branch
3. Merges all PRs into consolidation branch
4. Auto-resolves conflicts using "theirs" strategy
5. Creates ONE consolidated PR with checklist
6. Closes all original PRs with explanation
7. Consolidated PR proceeds through CI/CD
...
**Managed by:** Task deduplicator script
...
**Solution:** Check `task_state.json` for `last_orchestration` date. The workflow has a guard to prevent this, but if state file is corrupted, reset it:
```bash
...
2. Verify `task_state.json` is being committed
3. Check deduplication window (may need to increase)
...
2. Verify PRs don't have "consolidated" label
3. Run consolidator manually: `gh workflow run daily-pr-consolidator.yml`
...
2. Conflicts are auto-resolved using "theirs" strategy
3. Manual review recommended for conflict PRs
4. Original PRs listed in consolidated PR for reference
...
2. Let master orchestrator run once
3. Verify task state file created
4. Monitor consolidation on first run
5. Review and merge consolidated PR
6. Confirm old PRs are closed

### Rollback Plan
If issues occur:
1. Disable `daily-master-orchestrator.yml`
2. Disable `daily-pr-consolidator.yml`
3. Remove deduplication checks from `jules.yml` and `orchestrator.yml`
4. Delete `task_state.json`
5. Individual workflows continue as before
...
2. **Check task state weekly** - ensure deduplication working
3. **Monitor for conflicts** - review auto-resolved merges
4. **Adjust schedules** - optimize for your timezone
5. **Keep state file** - don't delete `task_state.json`
6. **Test changes** - use manual triggers to test workflows
...
3. Check `task_state.json` for state issues
