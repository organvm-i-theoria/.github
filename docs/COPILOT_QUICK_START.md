...
- ✅ **Custom instructions** configured for consistent, high-quality code generation
...
4. **Done!** Everything is pre-configured. Skip to [Verify Setup](#verify-setup)
...
4. **Continue to** [Language-Specific Setup](#language-specific-setup)
...
2. **Admin**: Verify files are in correct locations:
   - Instructions: `instructions/*.instructions.md`
   - Agents: `agents/*.agent.md`
   - Chat modes: `chatmodes/*.chatmode.md`

3. **Users**: Install GitHub Copilot in VS Code
4. **Users**: Clone any org repository - instructions apply automatically
...
3. **Verify**: Copilot should suggest code following your organization's standards
...
3. **Verify**: Should list relevant instructions
...
**Custom instructions**: Automatically applied to `**.ts`, `**.tsx`, `**.js` files
...
**See**: [Language-specific documentation](DEVELOPMENT_ENVIRONMENT_SETUP.md#language-specific-setup)
...
2. **Open a collection file**: e.g., `python-mcp-development.md`
3. **Click install badges**: Each component has an "Install in VS Code" badge
4. **Or install manually**: Copy files to your project
...
2. Use the generator prompt: `/python-mcp-server-generator`
...
4. Save to `.github/copilot-instructions/`
...
2. **Check license**: Ensure your Copilot subscription is active
3. **Restart VS Code**: Sometimes needed after configuration changes
4. **Check logs**: View > Output > GitHub Copilot

### Instructions Not Applying

1. **Check file patterns**: Ensure `applyTo` glob matches your files
2. **Verify location**: Instructions should be in `.github/copilot-instructions/`
3. **Check YAML frontmatter**: Must be valid YAML with `description` and `applyTo`
4. **Restart VS Code**: Reload window after adding instructions
...
2. **Verify server starts**: Test command manually in terminal
3. **Check logs**: Output panel for MCP-related errors
4. **Restart VS Code**: Reload window after configuration changes

**Still having issues?**
- Check detailed troubleshooting: [MCP Setup](MCP_SERVER_SETUP.md#troubleshooting) | [Custom Instructions](CUSTOM_INSTRUCTIONS_SETUP.md#troubleshooting) | [Dev Environment](DEVELOPMENT_ENVIRONMENT_SETUP.md#troubleshooting)
- Open an issue: [GitHub Issues](https://github.com/ivviiviivvi/.github/issues)
- Ask in discussions: [GitHub Discussions](https://github.com/orgs/ivviiviivvi/discussions)
...
2. **Use descriptive names**: Help Copilot understand intent
3. **Leverage chat**: Ask questions for complex problems
4. **Review suggestions**: Always verify generated code
5. **Provide examples**: Show patterns you want to follow

### Using Custom Instructions

1. **Start with essentials**: Don't over-constrain initially
2. **Iterate based on feedback**: Refine as you learn what works
3. **Be specific**: Clear rules are easier for Copilot to follow
4. **Include examples**: Show good and bad patterns
5. **Update regularly**: Keep instructions current

### Working with Agents

1. **Use the right agent**: Choose based on task domain
2. **Provide context**: Reference relevant files and goals
3. **Be specific**: Clear requests get better results
4. **Iterate**: Refine based on agent output
5. **Combine with chat**: Use both for complex tasks
...
2. ✅ **Verify everything works** with test examples
3. ✅ **Try an agent** for a real task
4. ✅ **Install collections** for your primary languages

### This Week

1. **Explore documentation** linked above
2. **Customize instructions** for your team's needs
3. **Build an MCP server** for a common task
4. **Share with team** and gather feedback

### This Month

1. **Create custom agents** for specialized domains
2. **Measure impact**: Track time saved and code quality
3. **Expand coverage**: Add more instructions and agents
4. **Contribute back**: Share improvements with the organization
...
- **Agent Registry**: [AGENT_REGISTRY.md](AGENT_REGISTRY.md) - Catalog of all agents
- **Contributing**: [CONTRIBUTING.md](CONTRIBUTING.md) - How to contribute

### Community

- **Questions**: [GitHub Discussions](https://github.com/orgs/ivviiviivvi/discussions)
