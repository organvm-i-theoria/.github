...
Comprehensive automation system for managing ALL pull requests - from
Dependabot, Jules, GitHub Actions, Copilot, Renovate, and any other automated
sources - with minimal manual intervention.

## 🎯 Overview

This system addresses common pain points in managing repositories with high PR
volume from ALL sources:

- ✅ **Automated PR batching** - Group PRs from Dependabot, Jules, and all
  automated tools
- ✅ **Nightly cleanup** - Automated maintenance of PRs, branches, and more
- ✅ **Draft-to-ready automation** - AI agent PRs (Jules, Copilot, etc.)
  auto-convert when ready
...
1. Finds all open automated PRs (Dependabot, Jules, GitHub Actions, Renovate,
   Copilot, etc.)
1. Groups them by source and category
1. Applies batch labels (e.g., `batch:jules`, `batch:dependabot-npm`)
1. Comments on each PR with batch information
1. Triggers batch merge if all PRs in a batch pass checks

**Supported automated sources:**

- **Dependabot** - Grouped by ecosystem (npm, pip, GitHub Actions, Docker, Go,
  PHP)
...
1. **merge-all-ready** - Merge all PRs that pass checks
1. **close-all-stale** - Close PRs inactive for 90+ days
1. **convert-all-drafts** - Convert all draft PRs to ready
1. **label-all-dependencies** - Label dependency PRs for auto-merge
...
- Now explicitly summons AI assistants (@copilot, @claude, @jules) when
...
**Why this was needed:** When the workflow programmatically converts a PR from
draft to ready using `github.rest.pulls.update`, GitHub does not trigger the
`ready_for_review` event. This meant workflows listening for that event (like
`auto-assign.yml` and `auto-enable-merge.yml`) would not run automatically.

Additionally, when users manually converted PRs from draft to ready via the
GitHub UI, the workflow didn't run at all because it wasn't listening for the
`ready_for_review` event. This caused AI assistants and integrated GitHub Apps
to not be triggered.

**Solution:** The workflow now:

1. Listens for the `ready_for_review` event (for manual conversions)
1. Explicitly performs notification actions for both automated and manual
   conversions
1. Ensures the full automation chain works correctly regardless of conversion
   method
...
1. Ready PRs are merged nightly
1. Drafts from AI agents auto-convert
1. Stale PRs are cleaned up
1. Copilot is assigned to all PRs
...
1. **PR Limits** - Prevent runaway operations (default: 50)
1. **Protected Branches** - Never auto-delete main/master/develop
1. **Skip Labels** - Manual override for any automation
1. **Concurrency Control** - Prevents simultaneous runs
...
1. **Use batch labels** - Group related PRs for efficient merging
1. **Monitor nightly runs** - Check summaries occasionally
1. **Set PR limits conservatively** - Increase only after testing
1. **Use skip labels** - For important PRs needing manual review
1. **Review dependabot batches** - Before they auto-merge
1. **Keep protected branches updated** - Add new long-lived branches
...
1. **Batch existing Dependabot PRs**:
...
1. Gradually enable more automation
...
1. Add safety checks for new operations
