...
2. Hash checked against recent tasks (24h window)
3. Duplicate tasks skipped with notification
4. Prevents cascade of redundant work

### PR Consolidation
1. Finds all PRs from bots/Jules/github-actions
2. Creates single consolidation branch
3. Merges all PRs (auto-resolves conflicts)
4. ONE PR with checklist for all changes
5. Closes original PRs with explanation
...
**Normal** - deduplication working correctly. Wait 24h or contact maintainer if urgent.
...
2. Verify no "consolidated" label on PRs
3. Run manually: `gh workflow run daily-pr-consolidator.yml`
...
**Remember:** The system runs daily at 1 AM UTC. Check your repos each morning for ONE consolidated PR to review!
