...
**Learning:** Found `MouthpieceFilter` contained duplicate logic (executing operations twice and overwriting results) and references to undefined attributes, likely due to a bad merge. Additionally, `_detect_intent` used slow iterative substring checks. Fixing the redundancy and switching to pre-compiled regex dictionary lookups provided a ~5x speedup.
**Action:** Always scan for "code rot" (duplicate lines) in legacy scripts. Replace `if any(w in text for w in list)` patterns with `_REGEX.search(text)` using pre-compiled patterns for significant performance gains.
